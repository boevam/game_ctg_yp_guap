...
restart_label_kvadrat = restart_label.get_rect(topleft=(235, 470))
...
# Функция для загрузки файла с рекордом
def load_high_score():
...
# функция сохранения рекорда в файл
def save_high_score(score):
    with open('high_score.txt', 'w') as file:
        file.write(str(score))


high_score = load_high_score() # загрузка рекорда в игру
...
player_speed = 15
player_x = 50  # координаты начала игрока
player_y = 450
jump = False
jump_count = 13
enemy_speed = 14  # начальная скорость врага
enemy_speed_increase_rate = 3  # на сколько увеличивать скорость каждые 5 врагов
...
while running:
    ...
    if gameplay:
        ...
        if vrag_list_in_game:
            ...
            for (i, el) in enumerate(vrag_list_in_game):
                ...
                if player_kvadrat.colliderect(el):
                    gameplay = False
                    # При проигрыше проверяем и сохраняем рекорд
                    current_score = enemies_jumped + enemies_killed
                ...
                if vrag_list_in_game:
                    for (index, vrag_el) in enumerate(vrag_list_in_game):
                        if el.colliderect(vrag_el):
                            ...
                            enemies_killed += 1
                            bullets.pop(i)
                            break  # выход из цикла после уничтожения снаряда
        ...
        # Отображение рекорда во время игры
        high_score_text = small_font.render(f"Лучший счёт: {high_score}", True, (103, 6, 38))

        screen.blit(jumped_text, (20, 70))
        screen.blit(killed_text, (20, 100))
        screen.blit(total_text, (20, 130))
        screen.blit(high_score_text, (20, 160))
    else:
        screen.fill((255, 189, 197))  # вывод фона при проигрыше
        screen.blit(lose_label, (300, 70))
        screen.blit(restart_label, restart_label_kvadrat)
        # Перепроверка и обновление рекорда при проигрыше
        current_score = enemies_jumped + enemies_killed
        if current_score > high_score:
            high_score = current_score
            save_high_score(high_score)
        ...
        high_score_text_lose = small_font2.render(f"Лучший счёт: {high_score}", True, (103, 6, 38))

        screen.blit(final_score_text, (380, 205))
        screen.blit(final_jumped_text, (380, 290))
        screen.blit(final_killed_text, (380, 340))
        screen.blit(high_score_text_lose, (380, 390))
